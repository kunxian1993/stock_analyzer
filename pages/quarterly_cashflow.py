...
layout = html.Div(
        children=[
                        ...
                        children=[
                                html.Div(
                                        children=[
                                                ...
                                                html.Div(
                                                        dcc.Input(
                                                                id='input-stock-A-q-CF',
                                                                placeholder='Ticker Symbol',
                                                                type='text',
                                                                className='Input-box'
                                                        )
                                                ),
                                                html.Button('SUBMIT', id='text-submit-button-A-q-CF', n_clicks=0, className='Submit-button'),
                                        ],
                                ...
                                html.Div(
                                        children=[
                                                ...
                                                html.Div(
                                                        dcc.Input(
                                                                id='input-stock-B-q-CF',
                                                                placeholder='Ticker Symbol',
                                                                type='text',
                                                                className='Input-box'
                                                        )
                                                ),
                                                html.Button('SUBMIT', id='text-submit-button-B-q-CF', n_clicks=0, className='Submit-button'),
                                        ],
                                        style={'width': '13%', 'height': '25%', 'font-size': '16px', 'margin-top': '5px'}
                                ),
                                html.Div(
                                        children=[
                                                ...
                                                dcc.Store(id='stock-data-q-CF'),
                                                html.Div(children="Metric", className="menu-title"),
                                                dcc.Dropdown(
                                                        id='yaxis-column-q-CF',
                                 ...
                                 html.Div(
                                        children=[
                                                ...
                                                dcc.Dropdown(
                                                        id='yaxis-column-denominator-q-CF',
                                                        value='None',
                                                        className="dropdown"
                                                )                                                
                                        ],
                ...
                html.Div(
                        html.Div(dcc.Graph(id='stock-bar-chart-q-CF'), className="card"),
                        className="wrapper"
                ),
                html.Div(
                        html.Div(dcc.Graph(id='stock-trend-chart-q-CF'), className='card'),
                        className='wrapper'
                ),
                html.Div(
                        html.Div(dash_table.DataTable(id='growth-table-q-CF'), className='card'),
                        className='wrapper'
                )
        ]
)
...
@callback(
    Output('yaxis-column-q-CF', 'options'),
    Output('yaxis-column-denominator-q-CF', 'options'),
    Output('stock-data-q-CF', 'data'),
    Input('text-submit-button-A-q-CF', 'n_clicks'),
    Input('text-submit-button-B-q-CF', 'n_clicks'),
    State('input-stock-A-q-CF', 'value'),
    State('input-stock-B-q-CF', 'value'))
        ...
        if n_clicks_A < 1 and n_clicks_B < 1:
                ...
                for selected_stock in [selected_stock_A, selected_stock_B]:
                        if selected_stock != None:
                                ...
                                df1 = stock_object.quarterly_cashflow
                                df1 = df1.transpose()
                                df1['Date']=df1.index
...
# callback for user interaction
@callback(
    Output('stock-bar-chart-q-CF', 'figure'),
    Output('stock-trend-chart-q-CF', 'figure'),
    Output('growth-table-q-CF', 'data'),
    Input('text-submit-button-A-q-CF', 'n_clicks'),
    Input('text-submit-button-B-q-CF', 'n_clicks'),
    Input('stock-data-q-CF', 'data'),
    Input('yaxis-column-q-CF', 'value'),
    Input('yaxis-column-denominator-q-CF', 'value'))
